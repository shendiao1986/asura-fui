...
public class PageLoader {
			...
			try {
				value = cache.get(key, new Callable<List<String>>(){
					...
					public List<String> call() throws IOException  {
						...
						//URL page_url = this.getClass().getClassLoader().getResource(page_directory+ "/" + key + ".xml");
						...
						if(page.exists()){
							...
							if(source_page_url != null){
							   //lines = Files.readLines(new File(source_page_url.getFile()), Charsets.UTF_8);
	...
	public static void loadConfigFromDirectory(String page_directory){
		try {
		     //URL page_url = PageLoader.class.getClassLoader().getResource(page_directory);
